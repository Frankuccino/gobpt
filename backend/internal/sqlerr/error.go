...
// We'll store all the database errors that we usually face

// Code describe a specific type of database error.
...
const (
	...
	// NotNullViolation is reported with a not null constraint would be violated.
...
// MapSeverity amps the severity string from the underlying database to a Severity
...
type Error struct {
	...
	// the name of the schema containing the object, if any.
	SchemaName string

	// TableName: if the error was associated with a specific table, the name of the table.
	TableName string

	// ColumnName: if the error was associatedw ith a specific table column,
