...
// ConvertPgError concerts a pgconn.PgError to our custom Error type
...
// generateErrorCode creates consistent error codes from database erros
...
func getEntityName(tableName, columnName string) string {
	// First priorty: column name logic (most relaible for FK relationships)
...
func extractColumnForUniqueViolation(constraintName string) string {
	...
	// Try alternate convention (table_comlumn_key)
