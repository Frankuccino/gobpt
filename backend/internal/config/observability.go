...
func (c *ObservabilityConfig) Validate() error {
	...
	if !validLevels[c.Logging.Level] {
		return fmt.Errorf("Invalid logging level: %s (mustbe one of: debug, info, warn, error)", c.Logging.Level)
	}

	// Validate slow query threshold
	if c.Logging.SlowQueryThreshold < 0 {
		return fmt.Errorf("logging slow_query threshold must be non-negative")
	}
